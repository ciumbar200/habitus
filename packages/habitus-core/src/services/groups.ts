...
export async function fetchMyGroupsWithMembership(profileId: string): Promise<MyGroupEntry[]> {
  ...
  const { data, error } = await getSupabase()
    .from("habitus_groups")
    .select("id, slug, name, creator_id, listing_id, city, zone, status, target_members, notes, created_at")
    .in("id", groupIds)
...
export async function createGroup(
  ...
  const city = input.city ? normalizeCitySlug(input.city) : "";
  const zone =
    city && input.zone ? normalizeZoneSlug(city, input.zone) : "";
  if (input.city && !city) {
    return { group: null, error: "Ciudad no válida." };
  }
  if (input.zone && city && !isValidCityZone(city, zone)) {
    return { group: null, error: "Zona no válida para la ciudad seleccionada." };
  }
