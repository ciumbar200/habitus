        ...
        <Link to={`/miembro/${host.slug}`} className="flex min-w-0 flex-1 items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-label-sm text-warm-slate">
              {host.roleTitle ?? es.property.hostLabel}
            </p>
          </div>
        </Link>
        <Link
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
        <Link to={`/miembro/${host.slug}`} className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-body-md text-warm-slate">{host.roleTitle ?? es.property.hostLabel}</p>
        ...
        >
          ...
          {es.matches.viewProfile}
        </Link>
      </div>
      <p className="mt-4 text-body-sm text-warm-slate">{es.property.hostHint}</p>
    </section>
  );
}
